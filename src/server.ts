...
export function createLogger(minLevel: LogLevel): GatewayLogFn {
...
export interface GatewayApp {
	...
	readonly log: GatewayLogFn;
	listen(port?: number, host?: string): Promise<{ port: number; host: string }>;
	close(): Promise<void>;
}

/**
 * Build the HTTP + WebSocket app without binding a port (testable).
 */
export function createApp(
	cfg: Config,
	log: GatewayLogFn = createLogger(cfg.LOG_LEVEL),
): GatewayApp {
	initAuth(cfg);
	const sessions = new SessionManager();

	const server = createServer((req, res) => {
		handleHttpRequest(req, res, cfg, sessions);
	});
	...
	const wss = new WebSocketServer({
		...
		verifyClient: (info, done) => {
			const auth = authenticateUpgrade(info.req, cfg);
			if (!auth.ok) {
	...
	return {
		...
		async listen(port = cfg.PORT, host = cfg.HOST) {
			...
			log(
				"info",
				`Audio gateway listening on ws://${host}:${boundPort} ` +
					`provider=${cfg.PROVIDER} format=${cfg.AUDIO_FORMAT} ` +
					`metrics=${cfg.METRICS_ENABLED} webrtc=${cfg.WEBRTC_INGRESS}`,
			);
